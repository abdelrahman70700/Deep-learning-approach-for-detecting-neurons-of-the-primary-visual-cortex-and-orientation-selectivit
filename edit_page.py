...
class model_DL_ML:
    ...
    def predictors(X, XF, t=False):
        ...
        yp = model_DL_ML.predictor(model, XF)
        modell = models.load_model('model2d.h5')
        ypp = model_DL_ML.predictor(modell, X)
        modelll = models.load_model('model3d.h5')
        yppp = model_DL_ML.predictor(modelll, np.expand_dims(X, axis=4))
        voting = yp + 1 - 1
    ...
    def reSize(maxI, minI):
        ...
        for dim in Dim:
            a, _, maxIN, minIN = model_DL_ML.imgProcessing(maxI, minI, dim)
            X, XF, loc, st = model_DL_ML.Cut(minIN, maxIN, dim[0], dim[1])
    ...
    def apply(path):
        ...
        Video, maxI, minI = model_DL_ML.vidProcessing(Video)
        dim = (126, 126)
        maxI, minI, maxIN, minIN = model_DL_ML.imgProcessing(maxI, minI, dim)
        model_DL_ML.prepareLabels("Captured.PNG", "neuroNumber9")
        # putting manual labels on the images
        X, XF, loc, st = Cut(minIN, maxIN)
        y = model_DL_ML.readLabels("New folder (3)", st)
        model_DL_ML.model_1d(XF, y)
        model_DL_ML.model_2d(X, y)
        model_DL_ML.model_3d(X, y)
        model_DL_ML.testing(X, XF, y)
        yp = model_DL_ML.predictors(X, XF)
        neurons = model_DL_ML.neuroSearch(yp, loc, maxI)
        model_DL_ML.neuroArt(minI, neurons)
        model_DL_ML.neuroArt(maxI, neurons)
        model_DL_ML.neuroArt(cv2.resize(cv2.imread("Captured.PNG"), dim), neurons)


    def application(path):
        Video = io.imread(path)
        Video, maxI, minI = model_DL_ML.vidProcessing(Video)  
        dim=model_DL_ML.reSize(maxI, minI)
        a, _, maxIN, minIN = model_DL_ML.imgProcessing(maxI, minI, dim)
        X, XF, loc, st = model_DL_ML.Cut(minIN, maxIN, dim[0], dim[1])    
        yp=model_DL_ML.predictors(X,XF)    
        neurons=model_DL_ML.neuroSearch(yp,loc,a)
        avgIntensity=model_DL_ML.neuroIntensity(Video,neurons,dim)
        mult=maxI.shape[0]/dim[0]
        model_DL_ML.neuroArt(minI,neurons,mult)
        model_DL_ML.neuroArt(maxI,neurons,mult)
        np.save('imagee.npy',maxI)
        np.save('neuro2.npy',np.array(neurons))
        np.save('multi_neuro.npy',np.array(mult))
        return avgIntensity
...
class run:    
    def start(self):
        ...
        if uploaded_file is not None:
            x=model_DL_ML.extract(uploaded_file)
            #x=extract(uploaded_file)
            split_tup = os.path.splitext(x)
            file_extension = split_tup[1]
            if file_extension=='.mp4':
                x=mp4Tif(x,"extracted/"+"video.tif")  
                image=model_DL_ML.application(x)
            else:
                image =model_DL_ML.application("extracted/"+x)
            np.save("data",image)
